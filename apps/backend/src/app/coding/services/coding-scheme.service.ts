 ...
 * CodingSchemeService
 *
 * Handles coding scheme retrieval, reference extraction, and codebook generation.
 *
 * Extracted from WorkspaceCodingService to improve maintainability.
 */
@Injectable()
export class CodingSchemeService {
  private readonly logger = new Logger(CodingSchemeService.name);
  ...
  /**
   * Generate an Excel codebook for the specified units in a workspace
   */
      ...
      if (missingsProfile) {
        ...
        if (profile?.missings) {
          try {
            ...
            if (Array.isArray(profileMissings)) {
            ...
            this.logger.error(`Error parsing missings from profile: ${e.message}`);
          }
        }
      }
