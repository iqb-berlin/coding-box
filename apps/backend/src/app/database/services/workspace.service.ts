...
export class WorkspaceService {
          ...
          if (err) {
            ...
            csv.parseStream(stream, {
              ...
              .on('end', () => {
                ...
                console.log('bookletLogs-LENGTH', this.createPersonList(bookletLogs).length);

                this.createPersonList(bookletLogs)
                  .map(person => this.assignBookletLogsToPerson(person, bookletLogs))
                  // .map(person => this.assignUnitLogsToBookletAndPerson(person, unitLogs))
                  .forEach(person => {
                    this.personsRepository.find(
                      { where: { group: person.group, code: person.code, login: person.login } }
                    )
                      .then(p => {
                        p.forEach(pp => {
                          const personWithUnitLogs = this.assignUnitLogsToBookletAndPerson(pp as Person, unitLogs);
                          personWithUnitLogs.booklets.forEach(b => {
                            b.units.forEach(u => {
                              console.log('u.logs', u.logs);
                            });
                          });
                          const booklets : TcMergeBooklet[] = pp.booklets as TcMergeBooklet[];
                          console.log('booklets', booklets.length);
                          const mappedBooklets = booklets.map(b => ({
                            ...b,
                            // units: personWithUnitLogs.booklets.find(pb => pb.id === b.id).units,
                            logs: person.booklets.find(pb => pb.id === b.id).logs
                          }));
                          this.personsRepository.update(pp.id, { ...person, booklets: mappedBooklets });
                        });
                      });
                  });

                // this.createPersonList(unitLogs)
                //   .map(person => this.assignUnitLogsToBookletAndPerson(person, unitLogs))
                //   .forEach(person => {
                //     person.booklets.forEach(booklet => {
                //       booklet.units.forEach(unit => {
                //         console.log('unitLogs', unit?.logs);
                //       });
                //     });
                //     this.personsRepository.find(
                //       { where: { group: person.group, code: person.code, login: person.login } }
                ...
                //       .then(p => {
                //         p.forEach(pp => {
                //           const booklets = pp.booklets as TcMergeBooklet[];
                //           const mappedBooklets = booklets.map(b => {
                //             const units = person.booklets;
                ...
                //             };
                //           });
                //           // console.log('booklets[0', booklets[0]);
                //           this.personsRepository.update(pp.id, { ...person, booklets: booklets });
                //         });
                //       });
                //   });
              });
  ...
  createPersonList(rows: Response[] | Logs[]): Person[] {
    // this.personsRepository.find({where: {}})
  ...
  assignBookletLogsToPerson(person: Person, rows: Log[]): Person {
    ...
    person.booklets = booklets;
    // console.log('person', person.booklets);
    return person;
  }

  assignUnitLogsToBookletAndPerson(person: Person, rows: Log[]): Person {
    const booklets = [];
    rows.forEach(row => {
      if (row.groupname === person.group && row.loginname === person.login && row.code === person.code) {
        ...
        person.booklets.forEach(booklet => {
          ...
          if (booklet.id === row.bookletname) {
            ...
            booklets.push({ ...booklet, units: units });
          }
        });
      }
    });
    // console.log('bookFFFFFFFFlets', booklets);
    person.booklets = booklets;
    return person;
  }
