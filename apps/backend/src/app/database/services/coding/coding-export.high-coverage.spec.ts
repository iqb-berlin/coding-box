...
describe('CodingExportService high coverage paths', () => {
  ...
  it('exports coding lists through stream and buffer wrappers', async () => {
    ...
    await expect(service.exportCodingResultsByVersionAsCsv(1, 'v1', 7, '', '', true, jest.fn(), true)).resolves.toBeDefined();
    await expect(service.exportCodingResultsByVersionAsExcel(1, 'v2', 7, '', '', false, jest.fn(), true)).resolves.toBeInstanceOf(Buffer);
  });
