...
describe('CodingPsychometricExportService', () => {
  ...
  ) => new CodingPsychometricExportService(
    {
          ...
          andWhere: jest.fn(
            (
              ...
              const statusMatch = condition.match(
                /response\.status_(v[123])/
              );
              if (statusMatch) {
                statusKey =
                  `status${statusMatch[1].toUpperCase()}` as
                  'statusV1' | 'statusV2' | 'statusV3';
              }
              if (parameters?.psychometricIgnoredStatuses) {
                ignoredStatuses =
                  parameters.psychometricIgnoredStatuses;
              }
              return queryBuilder;
            }
          ),
          ...
          getRawMany: jest.fn(async () => {
            if (grouped) {
              ...
              return Array.from(counts.entries())
                ...
                .map(([key]) => {
                  const [personId, unitName, variableId] =
                    key.split('\u001F');
                  return {
  ...
  it('uses selected missing codes and scores and parses multi-value categories', () => {
    ...
    expect(
      ...
      ).normalizeCode(-3, missingDefinitions)
    ).toBe(-98);
    expect(
      (
        service as never as {
          normalizeScore: (
            value: { code: number; score: number | null },
            code: number,
            definitions: unknown[]
          ) => number | null;
        }
      ).normalizeScore({ code: -3, score: null }, -98, missingDefinitions)
    ).toBe(0);
    ...
    ).parseCategories('[true,false,true]', {
      multiple: true,
      values: [
    ...
    expect(categories.map(category => category.value)).toEqual(['A', 'C']);

    const isCategoryEligible = (
      service as never as {
        isCategoryEligible: (code: number | null) => boolean;
      }
    ).isCategoryEligible.bind(service);
    expect(isCategoryEligible(0)).toBe(true);
    expect(isCategoryEligible(-97)).toBe(false);
    expect(isCategoryEligible(null)).toBe(false);
  });

  it('keeps category discovery bounded by the configured limit', () => {
    const service = createService({ items: [] }, []);
    const item = {
      variable: {
        values: Array.from({ length: 1000 }, (_value, index) => ({
    ...
    (
      service as never as {
        initializeMetricDefinitions: (
          items: unknown[],
          missingDefinitions: unknown[],
          maxCategoryCount: number
        ) => void;
      }
    ).initializeMetricDefinitions([item], [], 10);

    expect(item.categories.size).toBe(10);
    expect(item.categoryLimitExceeded).toBe(true);
  });

  it('preserves explicitly valid empty response categories', () => {
    const service = createService({ items: [] }, []);
    ...
    const categories = (
      ...
      values: [{ value: '', label: 'Leere Antwort' }]
    });
  ...
  it('uses a domain mean and optionally removes the current item score', () => {
    const service = createService({ items: [] }, []);
    ...
    ).getDomainScore.bind(service);

    expect(getDomainScore({ sum: 8, count: 3 }, 2, false)).toBeCloseTo(8 / 3);
    expect(getDomainScore({ sum: 8, count: 3 }, 2, true)).toBe(3);
    expect(getDomainScore({ sum: 2, count: 1 }, 2, true)).toBeNull();
  });
  ...
  it('excludes codes and scores with ignored statuses for the selected version', async () => {
    ...
    expect(
      v2Analysis.rows.find(row => row.type === 'SCORE')
    ).toEqual(expect.objectContaining({ n: 3 }));
    expect(
      v2Analysis.rows.some(
        row => row.type === 'CODE' && row.code === '99'
      )
    ).toBe(false);
    ...
    expect(
      v1Analysis.rows.find(row => row.type === 'SCORE')
    ).toEqual(expect.objectContaining({ n: 4 }));
  });
  ...
  it('uses one read-only repeatable-read snapshot for both response passes', async () => {
    ...
    expect(
      dependencies.workspaceExclusionService.resolveExclusionsForQueries
    ).toHaveBeenCalledTimes(1);
    expect(
      dependencies.connection.queryRunner.startTransaction
    ).toHaveBeenCalledWith('REPEATABLE READ');
    expect(dependencies.connection.queryRunner.query).toHaveBeenCalledWith(
      'SET TRANSACTION READ ONLY'
    );
    expect(
      dependencies.connection.queryRunner.commitTransaction
    ).toHaveBeenCalledTimes(1);
    expect(
      dependencies.connection.queryRunner.rollbackTransaction
    ).not.toHaveBeenCalled();
    expect(dependencies.connection.queryRunner.release).toHaveBeenCalledTimes(
      1
    );
  });
  ...
  it('reports VOMD items whose variableId cannot be resolved', async () => {
    ...
    const mapping = await (
      service as never as {
        buildItemMappingContext: (workspaceId: number) => Promise<{
          items: unknown[];
          issues: string[];
        }>;
      }
    ).buildItemMappingContext(7);
