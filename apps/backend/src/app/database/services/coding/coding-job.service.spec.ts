...
const createQueryBuilder = (result: unknown = []) => {
  ...
  qb.getCount = jest
    .fn()
    .mockResolvedValue(typeof result === 'number' ? result : 0);
  qb.execute = jest.fn().mockResolvedValue(result);
  return qb;
};

const expectManualCodingCandidateStatusFilter = (
...
describe('CodingJobService', () => {
  ...
  it('allows assigned coder access only when coding capability is active', async () => {
    ...
    await expect(
      service.assertUserCanAccessCodingJob(12, 7, 5)
  ...
  it('rejects duplicate assigned coder ids before saving assignments', async () => {
    await expect(
      service.assignCoders(12, [5, 5], undefined, 7)
  ...
  it('rejects DERIVE_ERROR manual coding opt-ins when the workspace setting is disabled', async () => {
    ...
    await expect(
      service.assertDeriveErrorManualCodingEnabled(3, {
        selectedVariables: [
  ...
  it('does not block usage calculations for existing DERIVE_ERROR opt-ins', async () => {
    ...
    await expect(
      service.calculateDistributionVariableUsageBatch(3, [
        {
  ...
  it('rejects missings profile changes after coding work exists', async () => {
    ...
    codingJobUnitRepository.createQueryBuilder.mockReturnValue(
      createQueryBuilder(1)
    );

    await expect(
      service.updateCodingJob(12, 7, { missingsProfileId: 77 })
  ...
  it('sets variable bundle ids on distributed bundle job units', async () => {
    ...
    jest
      .spyOn(service, 'getCurrentAggregationSettingsSnapshot')
      .mockResolvedValue({
  ...
  it('requires coding capability for coding actions even when the user has manager access', async () => {
    ...
    await expect(
      service.assertUserCanCodeCodingJob(12, 7, 5)
    ).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('marks a coding job open when every unit is coded or flagged open', async () => {
    (
      service as unknown as { getCodingJobProgress: jest.Mock }
    ).getCodingJobProgress = jest.fn().mockResolvedValue({
      progress: 50,
      coded: 5,
      total: 10,
      open: 5
    ...
    await (
      service as unknown as {
        checkAndUpdateCodingJobCompletion: (
          codingJobId: number
        ) => Promise<void>;
      }
  ...
  it('loads coding jobs with assignments, bundles and progress', async () => {
    ...
    codingJobVariableRepository.find.mockResolvedValue([
      { coding_job_id: 11, unit_name: 'UNIT', variable_id: 'VAR' }
    ]);
    codingJobVariableBundleRepository.find.mockResolvedValue([
      {
        coding_job_id: 11,
  ...
  it('filters coding jobs by assigned coder before loading assignments and progress', async () => {
    ...
    codingJobCoderRepository.createQueryBuilder.mockReturnValueOnce(
      assignedJobsQueryBuilder
    );
    codingJobCoderRepository.find.mockResolvedValueOnce([
      { coding_job_id: 11, user_id: 5 }
    ]);
    codingJobRepository.count.mockResolvedValue(1);
    ...
    expect(codingJobCoderRepository.createQueryBuilder).toHaveBeenCalledWith(
      'coder'
    );
    expect(assignedJobsQueryBuilder.innerJoin).toHaveBeenCalledWith(
      'coder.coding_job',
      'coding_job'
    );
    expect(assignedJobsQueryBuilder.where).toHaveBeenCalledWith(
      'coder.user_id = :userId',
      { userId: 5 }
    );
  ...
  it('counts coding jobs by job definition id', async () => {
    ...
    expect(codingJobRepository.createQueryBuilder).toHaveBeenCalledWith(
      'coding_job'
    );
  ...
  it('counts task deltas for retained cases in job definition refresh previews', async () => {
    ...
    codingJobUnitRepository.createQueryBuilder
      ...
      .mockReturnValueOnce(createQueryBuilder(0));
    codingJobRepository.createQueryBuilder.mockReturnValueOnce(
      createQueryBuilder({
        existingJobsCount: '2',
        staleJobsCount: '1'
      })
    ...
    await expect(
      service.previewJobDefinitionRefresh(3, {
        jobDefinitionId: 9,
        selectedVariables: [],
        selectedCoders: []
      })
  ...
  it('blocks job definition refresh when any coding work exists, including excluded units', async () => {
    ...
    expect(
      applyExclusionsSpy.mock.calls.some(
        call => call[2] === 'jobDefinitionCodingWork'
      )
    ).toBe(false);
  });
  ...
  it('builds the refresh distribution plan inside the locked transaction context', async () => {
    ...
    jest
      .spyOn(
        service as unknown as {
          getJobDefinitionExistingTaskRows: (...args: unknown[]) => Promise<[]>;
        },
    ...
    const buildPlanSpy = jest
      .spyOn(
        service as unknown as {
          buildDistributionPlan: (...args: unknown[]) => Promise<typeof plan>;
        },
        'buildDistributionPlan'
      )
    ...
    await expect(
      service.refreshDistributedCodingJobs(3, {
        jobDefinitionId: 9,
        selectedVariables: [],
        selectedCoders: []
      })
  ...
  it('throws when a coding job cannot be found', async () => {
    ...
    await expect(service.getCodingJob(99, 3)).rejects.toBeInstanceOf(
      NotFoundException
    );
  });
  ...
  it('creates, updates and deletes direct assignments', async () => {
    ...
    expect(codingJobCoderRepository.delete).toHaveBeenCalledWith({
      coding_job_id: 12
    });
    expect(result).toEqual([
  ...
  it('rejects transfer to the same coder', async () => {
    await expect(service.transferCodingCases(3, 1, 1)).rejects.toBeInstanceOf(
      BadRequestException
    );
  });
  ...
  it('rejects transfer when target user is not enabled as coder', async () => {
    ...
    await expect(service.transferCodingCases(3, 1, 2)).rejects.toBeInstanceOf(
      BadRequestException
    );
  ...
  it('returns assigned coding jobs and coder ids', async () => {
    ...
    await expect(service.getCodingJobsByCoder(4)).resolves.toEqual([
      { id: 1 },
      { id: 2 }
    ]);
  ...
  it('returns coding job details by id with assigned bundles', async () => {
    ...
    await expect(service.getCodingJobById(1)).resolves.toMatchObject({
      ...
      assignedVariables: [{ unitName: 'UNIT', variableId: 'VAR' }],
      assignedVariableBundles: [
        { name: 'Bundle', variables: [{ unitName: 'U2', variableId: 'V2' }] }
      ]
    });
  });
  ...
  it('rejects public updates that try to mark completed coding jobs as results applied', async () => {
    ...
    await expect(
      service.updateCodingJob(1, 3, { status: 'results_applied' })
    ).rejects.toBeInstanceOf(BadRequestException);
    expect(codingJobRepository.save).not.toHaveBeenCalled();
  });
  ...
  it('uses the provided transaction manager when marking coding job results applied', async () => {
    ...
    await expect(
      service.markCodingJobResultsApplied(1, 3, manager as never)
    ).resolves.toMatchObject({ status: 'results_applied' });
  ...
  it('does not include DERIVE_ERROR responses in slim variable selection', async () => {
    ...
    await expect(
      service.getSlimResponsesForVariables(3, [
        { unitName: 'UNIT', variableId: 'VAR' }
      ])
    ).resolves.toEqual([]);
  ...
  it('saves progress for coded and reopened units', async () => {
    ...
    expect(codingJobUnitRepository.save).toHaveBeenLastCalledWith(
      expect.objectContaining({
        code: null,
        score: null,
        coding_issue_option: null,
  ...
  it('rejects unsupported negative coding issue codes', async () => {
    ...
    ).rejects.toThrow('Unsupported coding issue code');
  ...
  it('maps saved coding progress and notes by composite key', async () => {
      ...
      .mockResolvedValueOnce([
        {
          ...
          data: {
            variableCodings: [
              { id: 'VAR', codes: [{ id: 1, code: 'A', label: 'Alpha' }] }
            ]
          }
        }
      ]);
    ...
    expect(
      progress['login@code@group@booklet::booklet::UNIT::GROUPED']
    ).toEqual({
      id: 2,
      code: undefined,
      label: undefined,
      ...
      .mockResolvedValueOnce([
        {
          ...
          data: {
            variableCodings: [
              { id: 'VAR', codes: [{ id: '7', code: 'S7', label: 'Scheme 7' }] }
            ]
          }
        }
      ]);
      ...
      .mockResolvedValueOnce([
        {
          ...
          data: {
            variableCodings: [
              { id: 'VAR', codes: [{ id: 0, code: 'S0', label: 'Scheme 0' }] }
            ]
          }
        }
      ]);
  ...
  it('normalizes and aggregates response values according to matching flags', () => {
    ...
    expect(
      service.aggregateResponsesByValue(responses, [
        ResponseMatchingFlag.IGNORE_CASE,
        ResponseMatchingFlag.IGNORE_WHITESPACE
      ])
    ).toMatchObject([
  ...
  it('returns resolved page overrides for coding job units', async () => {
    ...
    codingJobUnitRepository.find
      ...
      .mockResolvedValueOnce([]);
    codingFileCacheService.getVariablePageMap.mockResolvedValue(
      new Map([['VAR_WITH_OVERRIDE', '1']])
    );

    const result = await service.getCodingJobUnits(10);

    expect(codingFileCacheService.getVariablePageMap).toHaveBeenCalledWith(
      'UNIT',
      3
    );
