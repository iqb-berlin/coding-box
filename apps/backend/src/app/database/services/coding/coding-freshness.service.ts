...
export class CodingFreshnessService {
  ...
  async getWorkspaceStatusRevision(workspaceId: number): Promise<{
    ...
    const raw = await this.connection.query(
      `
        SELECT
          COALESCE(test_revision.revision, 0) AS revision,
          COALESCE(status_revision.revision, 0)::text AS status_revision,
          CASE
            WHEN status_revision.workspace_id IS NULL THEN false
            WHEN status_revision.processed_test_results_revision =
                   COALESCE(test_revision.revision, 0)
              AND status_revision.failed_test_results_revision IS NULL
              AND NOT EXISTS (
                SELECT 1
            ...
            ELSE false
          END AS stable
        FROM (VALUES ($1::integer)) requested_workspace(workspace_id)
        LEFT JOIN workspace_test_results_revision test_revision
          ON test_revision.workspace_id = requested_workspace.workspace_id
        LEFT JOIN workspace_coding_status_revision status_revision
          ON status_revision.workspace_id = requested_workspace.workspace_id
      `,
      [workspaceId]
    ) as Array<{
    ...
    return {
      revision: Number(raw[0]?.revision || 0),
      statusRevision: raw[0]?.status_revision || '0',
      stable: raw[0]?.stable === true
    };
  }
  ...
  private async upsertRows(
    ...
    for (const chunkRows of this.chunk(rows, this.FRESHNESS_UPSERT_BATCH_SIZE)) {
      await freshnessRepository.upsert(chunkRows, {
        conflictPaths: ['workspace_id', 'unit_id', 'version'],
        overwriteCondition: {
          where: '"coding_unit_freshness"."source_revision" <= EXCLUDED."source_revision"'
        }
      });
    }
