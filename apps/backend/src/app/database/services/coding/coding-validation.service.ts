  ...
  private async computeVariableCaseInfo(
    ...
    for (const variable of variables) {
      ...
      const countAggregatedCases = (responses: SlimCodingResponse[]) => {
        ...
        const dedupedResponses = deduplicateManualCodingResponses(
          responsesWithCaseFields
        );
        const assignedDedupedResponseIds = new Set(
          dedupedResponses
            .filter(
              response => assignedResponseIds.has(response.responseId) ||
                assignedDeduplicationKeys.has(
                  getManualCodingDeduplicationKey(response)
                )
            )
        ...
        for (const group of aggregatedGroups) {
          if (
            ...
            group.responses.length >= aggregationThreshold
          ) {
            uniqueCases += 1;
            if (
              group.responses.some(response => assignedDedupedResponseIds.has(response.responseId)
              )
            ) {
              casesInJobs += 1;
            }
            ...
            casesInJobs += group.responses.filter(response => assignedDedupedResponseIds.has(response.responseId)
            ).length;
          }
        }
