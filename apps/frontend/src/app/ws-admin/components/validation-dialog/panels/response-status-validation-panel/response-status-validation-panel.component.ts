...
export class ResponseStatusValidationPanelComponent implements OnInit, OnDestroy {
  ...
  onValidate(): void {
    ...
    this.isRunning = true;
    this.subscription = this.responseStatusValidationService.validate(this.currentPage, this.pageSize).subscribe({
      next: result => {
  ...
  onPageChange(event: PageEvent): void {
    ...
    this.subscription?.unsubscribe();
    this.subscription = this.responseStatusValidationService.fetchPage(this.currentPage, this.pageSize).subscribe({
      next: result => {
        this.invalidStatusVariables = result.data;
        this.totalInvalid = result.total;
        this.currentPage = result.page;
  ...
  deleteSelected(): void {
    ...
    this.responseStatusValidationService.deleteSelected(Array.from(this.selectedResponses)).subscribe({
      next: () => {
