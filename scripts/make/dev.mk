...
dev-config:
	docker compose --env-file $(CODING_BOX_BASE_DIR)/.env.dev config $(SERVICE)
...
dev-volumes-clean:
	if test "$(shell docker volume ls -f name=coding-box -q)";\
		then docker volume rm $(shell docker volume ls -f name=coding-box -q);\
	fi

## Remove all unused (not just dangling) images!
dev-images-clean:
	if test "$(shell docker images -f reference=coding-box-* -q)";\
		then docker rmi $(shell docker images -f reference=coding-box-* -q);\
	fi
