...
dev-db-up:
	...
	docker compose --env-file $(CODING_BOX_BASE_DIR)/.env.dev up --no-build --pull never -d db liquibase
...
dev-db-generate-docs:
	cd $(CODING_BOX_BASE_DIR)/database/changelogDocs && rm -vrf !(".gitignore") && cd $(CODING_BOX_BASE_DIR) &&\
	docker compose run --rm liquibase\
		liquibase\
				--headless=true\
				--show-banner=false\
