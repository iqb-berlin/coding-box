...
load_environment_variables() {
  # Load current environment variables in .env.coding-box
...
update_files() {
  ...
  download_file "$PWD"/docker-compose.coding-box.yaml docker-compose.yaml
  download_file "$PWD"/docker-compose.coding-box.prod.yaml docker-compose.coding-box.prod.yaml
  download_file "$PWD"/scripts/make/coding-box.mk scripts/make/prod.mk
...
get_modified_file() {
  ...
  current_env_file=.env.coding-box
...
check_environment_file_modifications() {
  ...
  get_modified_file .env.coding-box.template .env.coding-box.template "env-file"
...
update_application_infrastructure() {
  ...
  if [ -z "$TRAEFIK_DIR" ]; then
    ...
    if grep TRAEFIK_DIR= .env.coding-box >/dev/null; then
      sed -i "s#TRAEFIK_DIR.*#TRAEFIK_DIR=$TRAEFIK_DIR#" .env.coding-box
    else
      printf '\n# Infrastructure\nTRAEFIK_DIR=%s\n' "$TRAEFIK_DIR" >>.env.coding-box
    fi
