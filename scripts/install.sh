...
download_files() {
  ...
  download_file "config/keycloak/${APP_NAME}-realm.config" "config/keycloak/realm/${APP_NAME}-realm.config"
...
import_keycloak_realm() {
  ...
  declare keycloak_admin_name keycloak_admin_password keycloak_db_name keycloak_db_root keycloak_db_root_password
  ...
  docker run \
      ...
      --env-file "config/keycloak/${APP_NAME}-realm.config" \
      --volume "${PWD}/config/keycloak/${APP_NAME}-realm.json:/opt/keycloak/data/import/${APP_NAME}-realm.json" \
      --network app-net \
    quay.io/keycloak/keycloak:22.0 start --http-enabled true --import-realm >/dev/null
  docker stop keycloak-coding-box-realm-import >/dev/null
  docker rm -v keycloak-coding-box-realm-import >/dev/null
